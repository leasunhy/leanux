...
void page_init() {
    ...
    /* map mem below 1MiB read-write */
    for (PAGE *i = 0x0; i < &KERNEL_RO_START; ++i)
        kernel_pt[pt_addr(i)] = make_pte(pt_addr(i), true, false, true);

    /* map readonly part of kernel */
    for (PAGE *i = &KERNEL_RO_START; i < &KERNEL_RO_END; ++i)
        kernel_pt[pt_addr(i)] = make_pte(pt_addr(i), true, false, false);

    /* map read-write part of kernel */
    for (PAGE *i = &KERNEL_RW_START; i <= &KERNEL_RW_END; ++i)
        kernel_pt[pt_addr(i)] = make_pte(pt_addr(i), true, false, true);
...
void mm_init(uint32_t max_kb_installed) {
    ...
    for (uint32_t i = kernel_break >> 12; i <= kernel_page_break; ++i)
        /*mm_mmap(kernel_pd, i, i, true, false, true);*/
        kernel_pt[i] = make_pte(i, true, false, true);
    kernel_break += memmap_size;
    for (uint32_t i = 0; i <= kernel_page_break; ++i)
        mem_map[i] = 1;
    for (uint32_t i = kernel_page_break + 1; i < num_of_page; ++i)
        mem_map[i] = 0;
}
...
void mm_mmap(PDE *pd, uint32_t vir_page, uint32_t phy_page,
    ...
    if (!(pd[pde_no] & 1)) {
        /* if not, allocate one */
        uint32_t new_pt_page = alloc_page(0, 4095);
        /* inform kernel */
        mm_mmap(kernel_pd, new_pt_page, new_pt_page, true, false, true);
        memory_set((void*)(new_pt_page << 12), 0, 4096);
        pd[pde_no] = make_pde(new_pt_page, user, read_write);
    }
    PTE *pt = (PTE*)pd[pde_no];
